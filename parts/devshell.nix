  ...
  options = {
    perSystem = mkPerSystemOption (_: {
      ...
      options.beamWorkspace = mkSubmoduleOptions {
        devShell = {
          enable = mkEnableOption "beam-flakes devshells" // {
            default = true;
          };
          extraArgs = mkOption {
          ...
          iexShellHistory = mkEnableOption "IEx shell history" // {
            default = true;
          };
          packages = mkOption {
            internal = true;
          ...
          default = pkgs.mkShell (
            {
              ...
              ERL_AFLAGS = if cfg.devShell.iexShellHistory then "-kernel shell_history enabled" else null;
            }
